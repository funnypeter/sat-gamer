...
- `lib/collegeboard/transform.ts` — converts CB's HTML question shape into our `questions` row shape:
  ...
  - **HTML cleanup**: strips CB's accessibility spans (`<span aria-hidden="true">______</span><span class="sr-only">blank</span>`) to plain `______` so the screen-reader-only "blank" text doesn't leak into the visible passage.
